document.addEventListener('DOMContentLoaded', function() {
    ...
    // =============================================================
    // --- 2. LOGICA DI ACQUISTO FLESSIBILE ---
    // =============================================================
        ...
        finalBtn.addEventListener('click', () => {
            ...
            const checkoutData = {
                items: [{
                    ...
                    price: basePrice,
                    addSIAE: modalContent.querySelector('.upsell-item[data-id="add-siae"]')?.classList.contains('selected') || false,
    ...
    function redirectToCheckout(data) {
        ...
        for (const item of data.items) {
            ...
            if (isNaN(unitAmount) || unitAmount <= 0) {
                ...
                alert(`Errore: il prezzo per il brano "${item.title}" non è valido.`);
    ...
    function setupEventListeners() {
        ...
        document.getElementById('cart-checkout-btn').addEventListener('click', () => {
            ...
            const checkoutData = { items: shoppingCart };
            redirectToCheckout(checkoutData);
        });
        ...
        document.getElementById('show-gadget-modal-btn').addEventListener('click', () => document.getElementById('contact-modal').style.display = 'flex');
    ...
    async function handleContactForm(e) {
        ...
        try {
            ...
            if (result.success) {
                formResult.innerHTML = "<span style='color: #4CAF50;'>Inviato!</span>";
                contactForm.reset();
                setTimeout(() => {
            ...
            formResult.innerHTML = "<span style='color: #e53935;'>Errore.</span>";
        } finally {
    ...
    async function handleLogin(e) { 
        ...
        btn.disabled = true; 
        try { 
            const url = `${sheetApiUrl}&emailCheck=${encodeURIComponent(email)}`;
    ...
    async function loadMusicFromApi(userEmail) { 
        ...
        try { 
            const url = `${sheetApiUrl}&userEmail=${encodeURIComponent(userEmail)}`;
            const res = await fetch(url); 
            if (!res.ok) throw new Error('Errore di rete.'); 
